...
%% Load the bangal_vowel data
% The provided bangla_vowel data is saved in a zip file as a python _pickle_
% object. In order to use it in Matlab, the data is converted into |.mat|
% file. In this case, the bangla_vowel data has been saved as a |.mat| file.
...
training_inputs = double(bangla_vowel{1,1}');                          % transpose the matrix to make the pixel data as the row element
training_results = vectorizeData(bangla_vowel{1,2});                   % convert the digit into the activation value of the neural network

validation_inputs = double(bangla_vowel{2,1}');
validation_results = bangla_vowel{2,2};

test_inputs = double(bangla_vowel{3,1}');
test_results = bangla_vowel{3,2};
...
n(1) = size(bangla_vowel{1,1},2);                                      % nodes at the input layer, number of pixel square

for i = 1:length(nh)
    n(1+i) = nh(i);                                             % nodes at the hidden layers
end

n(l) = 11;                                                      % nodes at the output layer, number of digit
